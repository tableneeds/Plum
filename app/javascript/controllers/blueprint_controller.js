...
export default class extends Controller {
  ...
  addField(event) {
    ...
    const fieldHtml = `
      <div class="flex items-center space-x-4 p-4 bg-gray-50 rounded-lg" data-blueprint-target="field">
        <input type="text" placeholder="handle" data-field="handle"
               data-action="input->blueprint#inputChanged"
        ...
        <select data-field="type" data-action="change->blueprint#inputChanged"
        ...
        <button type="button" data-action="blueprint#removeField" class="text-red-500 hover:text-red-700">
  ...
  removeField(event) {
    event.preventDefault()
    const fieldDiv = event.currentTarget.closest('[data-blueprint-target="field"]')
    fieldDiv.remove()
    this.updateHiddenField()
  }
  ...
  updateHiddenField() {
    const fields = []
    const fieldElements = this.fieldsTarget.querySelectorAll('[data-blueprint-target="field"]')
